...
`distractions setup --remove` reverses setup: firewall/helper/grant/install-record removal, user slice cleanup, owned notification-clone cleanup, launcher restoration and previous-browser restoration, and removal of the marked Hyprland line and the file setup wrote (an edited file is moved into the state directory's backup). Removal unlinks only regular files it installed: a symlink standing where the slice unit or a WirePlumber file was is left in place and reported, never followed. The owner then removes the plugin through Omarchy. User data such as profiles is not described as automatically erased.
