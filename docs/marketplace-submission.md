...
`distractions setup` is the only place privilege enters. It asks for the sudo password interactively, once, and performs two root-owned writes through `sudo sh -c 'cmp -s "$1" "$2" || install ...'`: the wrapper `distractions-nft` from the repository root to `/usr/local/libexec/omarchy-distraction-space/distractions-nft`, mode 0755, and a sudoers drop-in to `/etc/sudoers.d/omarchy-distraction-space`, mode 0440. The drop-in is `install/sudoers.omarchy-distraction-space` with `__INSTALL_USER__` replaced by the installing account: one user, NOPASSWD as root, on that one absolute path, and nothing else. Setup renders it to a private temp file, runs `visudo -cf` on it, and installs only a file visudo accepted. It refuses to run when any ancestor of either destination is writable by the user, refuses principal names that are empty, `ALL`, a `%group`, or contain sudoers metacharacters, and never reads or lists `/etc/sudoers.d`.

The wrapper is a short Python script with no options. It accepts exactly two arguments, `replace ds` or `flush ds`, and refuses any other argv. `replace` reads addresses on stdin, one per line: each token must parse with Python's `ipaddress.ip_address` and is refused when it contains `/` or `\`, so no prefix, path, or nft syntax reaches the ruleset, and duplicates are dropped. `flush` refuses any stdin at all. The validated addresses are the only variable content in a fixed nft script that destroys and recreates the table `inet omarchy_ds`: two address sets, an output filter chain that rejects traffic to set members, and an output nat chain that redirects their ports 80 and 443 to the plugin's local routers on 28080 and 28443. The wrapper refuses to commit a script that does not name that table, then feeds it to `nft -f -`. The listener calls it with `sudo -n` every 30 seconds while the user is off the space, and with `flush ds` when the user enters it.
...
- `curl-pipe-shell`: not triggered. No `curl` or `wget` in any runtime file. The only `curl` strings in the tree are plain `curl https://...` live-check commands in planning notes under `.flow/specs/`, never piped to a shell or written to an executed file. Installation is `omarchy plugin add` alone (README.md, Install).
...
- `installer`: expected. `ds/setup.py` is a setup-named path and runs `install -D` and `install -m` through sudo (ds/setup.py:60 to 71); README.md "Install" documents it. This is the intended one-time `distractions setup`.
...
- `sudoers-modification`: expected. install/sudoers.omarchy-distraction-space is the policy file; ds/setup.py validates it with `visudo -cf` (line 379), installs it to `/etc/sudoers.d/omarchy-distraction-space` (line 390), and removes it on `--remove` (line 420); README.md lines 31 and 86 document the write. The helper the policy names is `distractions-nft`, a root-owned script with a fixed two-word command surface and stdin restricted to parsed IP addresses, which is the shape SECURITY.md sets aside for manual review rather than rejection.

Scan scope note: the tracked `.flow/` directory holds planning specs, task records, and review receipts as Markdown and JSON. The JSON files are configuration-format text and fall inside the scan; they mention `sudo`, `pkexec`, and `NOPASSWD` as words in prose about this plugin's design, which adds nothing beyond the `privilege` and `sudoers-modification` capabilities already expected from the runtime files.
