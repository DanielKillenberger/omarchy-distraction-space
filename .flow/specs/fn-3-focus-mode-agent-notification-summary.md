...
**Ping-text capture (this spec owns it).** Planned mute stores only `{ "<app-label>": <int> }` and discards individual banners. In the same toast handler, before mute dismisses the toast or deletes history, the child sends `{app,title,body,at}` on stdin to a silent `distractions capture-ping` helper while focus is on and `agent_summaries` is true. That helper takes the session flock, rejects stale/nonfocused sessions, atomically allocates the next session-monotonic `seq` from the control file, appends the complete `{seq,app,title,body,at}` record, and trims oldest records under the same lock to keep JSONL at 64 KiB. Trimming never renumbers survivors. QML never reads or rewrites the JSONL directly; only `capture-ping` mutates it. Membership is mute's identity map only. If that map is not in tree yet, capture writes nothing and the session is empty ping-text (R4). Do not invent a second membership list. The writer does not dismiss the toast and does not increment the mute count.

**Parser location and start.** The parser is the Python subcommand `summarize-session` on the existing helper. It is a long-lived flocked singleton, separate from `listen()`. Focus-on first takes the session lock, writes a new session id and control state (unless a lift-fail catch-up is pending), and reaps any leftover agent child via the session pidfile. It then flips the focus flag and applies mute. Only after mute apply succeeds does it atomically set the session-ready marker. The service starts its Quickshell `Process` only for a current ready session when `agent_summaries` is true, so a focus-flag transition cannot bind the parser to stale control state. The QML `Process` is the activator. If the process exits unexpectedly while focus is still on, the service starts `summarize-session --restart` after 1 s, then 4 s. On that entry path Python takes the session flock and atomically increments durable `parser_restarts` before initialization; it refuses to run when the value exceeds two. On service startup, an uncleared parser-active marker for a current ready session selects the same `--restart` path. The service only reads restart state and never increments it, so a Quickshell restart cannot reset or double-count the cap. A clean `summarize-finish`, mid-session disable, or finish-request marker clears active/ready state and never restarts. `listen()` restart does not own or kill the session. If the service is not loaded, capture and parse do not run and R4 applies.

The session watches the JSONL file (inotify, or a 250 ms poll if inotify is missing). Each record has the session-monotonic `seq` allocated by `capture-ping`. A mode-`0600`, session-keyed control file atomically stores `next_seq`, `invocations`, `last_consumed_seq`, `parser_restarts`, parser-active/session-ready state, and the finish-request marker. A fresh session seeds `next_seq = 1`; a retained lift-fail session and every parser/Quickshell restart reload it. Before every agent spawn, under the session flock, reserve one invocation and advance `last_consumed_seq` through the records included in that prompt; a crash therefore consumes budget and cannot replay paid input. Restarts reload this state. The first unseen record starts the first one-shot. Later unseen records follow the replacement rules. `capture-ping` is the only record handoff; there is no separate parser-kick subcommand.
...
**Focus-on.** Under the session lock, allocate the new session/control state and cancel leftover agent children unless a lift-fail catch-up is pending. Discard unread stdout on that clean reset. Then flip the focus flag, apply mute (when present), and apply any network block. After mute succeeds, publish the session-ready marker atomically. Arm capture if summaries are on. The service starts `summarize-session` only from that ready marker, never from the focus flag alone.
...
- `capture-ping` reads one bounded `{app,title,body,at}` object from stdin, takes the session flock, assigns durable `seq`, appends and trims the JSONL atomically, and prints nothing.
...
**D11 · parser start and capture ownership.** `summarize-session` lives in Python. Focus-on prepares session state before the focus flag and publishes a session-ready marker only after mute apply; the service starts the parser from that marker via Quickshell `Process`. Silent `capture-ping` is the only JSONL writer and owns locked `seq` allocation and trimming. The mode-`0600` control file durably owns `next_seq`, invocation reservations, consumed `seq`, and a two-restart backoff budget. Only `summarize-session --restart` increments the crash counter under lock; the service only reads it. Rejected. Putting the parser in `listen()`, direct QML JSONL rewrite, or waiting for a first-ping kick with no owner.
